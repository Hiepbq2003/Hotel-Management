...
const ALLOWED_ACCESS_ROLES = ['ADMIN', 'MANAGER'];
...
function EditUserModal({ show, handleClose, user, handleSave, editableRoles }) {
    ...
    const isTargetUserManager = user?.role === 'MANAGER';
            ...
            <Modal.Body>
                ...
                {isTargetUserManager && (
                    <Alert variant="danger">
                        Đây là tài khoản MANAGER duy nhất. Không ai có quyền thay đổi thông tin của tài khoản này!
                    </Alert>
                )}
                ...
                <Form>
                    ...
                    <Form.Group className="mb-3">
                        ...
                        <Form.Select
                            ...
                            value={formData.role}
                            onChange={handleChange}
                            disabled={isTargetUserManager}
                        >
                            {isTargetUserManager ? (
                                <option value="MANAGER">MANAGER</option>
                            ) : (
                                editableRoles.filter(r => r !== 'MANAGER').map(role => (
                                    <option key={role} value={role}>{role}</option>
                                ))
                            )}
                        </Form.Select>
                        {isTargetUserManager && <Form.Text className="text-danger">Không thể thay đổi vai trò của tài khoản Manager.</Form.Text>}
                    </Form.Group>
                    ...
                    <Form.Group className="mb-3">
                        ...
                        {isTargetUserManager && <Form.Text className="text-danger">Không thể thay đổi trạng thái của tài khoản Manager.</Form.Text>}
                    </Form.Group>
                </Form>
            </Modal.Body>
            <Modal.Footer>
                ...
                <Button
                    ...
                    disabled={!user || isTargetUserManager || originalDetailsMatch(user, formData)}
                >
                    Lưu Thay Đổi
                </Button>
            </Modal.Footer>
...
function UserManagement() {
    ...
    useEffect(() => {
        if (!ALLOWED_ACCESS_ROLES.includes(currentUserRole)) {
            setError('Bạn không có quyền truy cập trang Quản lý Người dùng. Chỉ Admin hoặc Manager mới được truy cập.');
            setLoading(false);
            return;
        }
        fetchUsers();
    ...
    const isCurrentUserAdminOrManager = ALLOWED_ACCESS_ROLES.includes(currentUserRole);

    const canEdit = (targetUserRole, targetUserId) => {
        if (!isCurrentUserAdminOrManager) return false;
        return targetUserRole !== 'MANAGER'; // Manager is completely uneditable
    };

    const getEditableRoles = () => {
        if (isCurrentUserAdminOrManager) {
            return ALL_STAFF_ROLES.filter(role => role !== 'MANAGER');
        }
        return [];
    };
    ...
    const handleCreateUser = async (formData) => {
        ...
        const payload = {
            ...formData,
            role: formData.role ? formData.role.toLowerCase() : 'reception'
        };
    ...
    const handleEditDetails = async (userId, updatedFields, originalUser) => {
        ...
        try {
            ...
            if (originalUser.status !== updatedFields.status && originalUser.role !== 'MANAGER') {
                updateStatus = true;
    ...
    return (
        ...
        <Container className="my-5">
            ...
            {!error && isCurrentUserAdminOrManager && (
                <Row className="mb-3">
                    <Col className="d-flex justify-content-start">
                        {/* Nút Tạo User Của Admin/Manager */}
                        <Button
                            variant="success"
                                            ...
                                            {isCurrentUserAdminOrManager ? (
                                                <>
                                                    <Button
                                                        ...
                                                        title={editable ? "Chỉnh sửa Tên, Vai trò và Trạng thái" : "Không thể sửa Manager"}
                                                        disabled={!editable}
                                                    >
                                                    ...
                                                    <Button
                                                        ...
                                                        title={editable ? "Reset mật khẩu về 123456" : "Không thể reset mật khẩu Manager"}
                                                        disabled={!editable}
                                                    >
